...
def corpus_bleu(refs_list, pred_list, n=4):
    ...
    for i, refs in enumerate(refs_list):
        assert isinstance(refs, (list, tuple)), (
            f"Got {type(refs)} at index {i}, expected list or tuple")
        assert len(refs) > 0, f"Got empty reference at index {i}"
        for j, ref in enumerate(refs):
            assert isinstance(ref, str), (
                f"Got {type(ref)} at index {i} reference {j}, expected string")
    for i, pred in enumerate(pred_list):
        assert isinstance(pred, str), (
            f"Got {type(pred)} at index {i}, expected string")
...
# hyp = "on the mat there is a cat"
# for n in range(1, 5):
#     weights = (1 / n,) * n + (0,) * (4 - n)
#     print(f"bleu {n}:", corpus_bleu([[ref]], [hyp], weights=weights))
